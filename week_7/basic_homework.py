...
wandb.run.name = 'bagic_homework'

@dataclass
class Arguments:
    model_name_or_path: Optional[str] = field(default="openai-community/openai-gpt")  # HuggingFace hub에서 pre-trained 모델로 사용할 모델의 이름
    torch_dtype: Optional[str] = field(default="auto", metadata={'choices': ['auto', 'bfloat16', 'float16', 'float32']})  # 우리 모델의 precision(data type이라고 이해하시면 됩니다)
    ...
    num_workers: Optional[int] = field(default=4)  # Data를 업로드하거나 전처리할 때 사용할 worker 숫자
...
trainer = Trainer(
    ...
    eval_dataset=test_dataset,  # [MY_CODE] test_dataset 추가
    tokenizer=tokenizer,
